...
**CRITICAL**: All code generated by Claude Code MUST refer to the relevant documentation files within the `/docs` directory.

Before implementing any feature or writing code:
1. **Check for existing documentation** in the `/docs` directory
2. **Reference the appropriate docs file** for implementation guidance
3. **Follow patterns and best practices** outlined in the documentation
4. **Never implement from memory alone** - always verify against the docs

This ensures consistency, accuracy, and adherence to project-specific patterns.
